...
function createTestApp(
  overrides: PromptApiDependencies,
): express.Express {
  const app = express();
...
describe('Prompt API routes', () => {
  ...
  it('returns aggregated assistant text for JSON chat requests', async () => {
    ...
    const spawnProcess = ((
      ...
      capturedCwd =
        typeof rawCwd === 'string' ? rawCwd : rawCwd?.toString();
    ...
    const response = await request(app).post(PROMPT_API_OPENAI_COMPLETIONS_ROUTE).send({
      messages: [
  ...
  it('lists and updates the default prompt API model', async () => {
    ...
    const currentResponse = await request(app).get(PROMPT_API_CURRENT_MODEL_ROUTE);
    ...
    const chatResponse = await request(app).post(PROMPT_API_OPENAI_COMPLETIONS_ROUTE).send({
      messages: [
  ...
  it('logs in and switches credentials without requiring chat requests to specify them', async () => {
    ...
    const spawnProcess = ((
      ...
      if (isolatedHome) {
        const oauthCredsPath = path.join(isolatedHome, '.gemini', 'oauth_creds.json');
    ...
    expect(loginResponse.body.login.redirectUri).toContain(
      'http://127.0.0.1:',
    );
    ...
    const credentialsResponse = await request(app).get(PROMPT_API_CREDENTIALS_ROUTE);
  ...
  it('returns quota data for all credentials or a specific credential', async () => {
    ...
    await credentialStore.createCredential('Secondary Google', 'cred-secondary');

    vi
      .spyOn(CodeAssistServer.prototype, 'loadCodeAssist')
      .mockResolvedValue({
    ...
    vi
      .spyOn(CodeAssistServer.prototype, 'retrieveUserQuota')
      .mockResolvedValue({
        buckets: [
          {
  ...
  it('keeps quota totals nullable when only remainingFraction is returned', async () => {
    ...
    const response = await request(app).get(PROMPT_API_QUOTA_ROUTE.replace(
      ':credentialId',
      credential.id,
    ));
  ...
  it('serves the management console and deletes credentials through the API', async () => {
    ...
    const credentialsAfterDelete = await request(app).get(PROMPT_API_CREDENTIALS_ROUTE);
    ...
    const deleteAllResponse = await request(app).delete(PROMPT_API_CREDENTIALS_ROUTE);
