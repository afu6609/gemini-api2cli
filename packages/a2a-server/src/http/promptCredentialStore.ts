...
export class PromptCredentialStore {
  constructor(private readonly rootDir: string = getDefaultPromptCredentialStoreRoot()) {}
  ...
  async listCredentials(): Promise<PromptApiCredentialRecord[]> {
    ...
    return credentials
      .filter((credential): credential is PromptApiCredentialRecord => credential !== undefined)
  ...
  async deleteCredential(credentialId: string): Promise<void> {
    ...
    await rm(this.getCredentialDir(credentialId), { recursive: true, force: true });
  ...
  private async writeState(state: PromptCredentialStoreState): Promise<void> {
    ...
    await writeFile(this.getStatePath(), JSON.stringify(state, null, 2), 'utf8');
  }
...
function getDefaultPromptCredentialStoreRoot(): string {
  return path.join(osHomedir(), GEMINI_DIR_NAME, 'prompt-api');
}
