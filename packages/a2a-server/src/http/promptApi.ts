...
export const PROMPT_API_GEMINI_STREAM_ROUTE = '/v1/gemini/streamGenerateContent';
export const PROMPT_API_OPENAI_COMPLETIONS_ROUTE = '/v1/openai/chat/completions';
...
function getSourceGeminiCliHome(
  sourceGeminiCliHome?: string,
): string {
  return sourceGeminiCliHome ?? process.env['GEMINI_CLI_HOME'] ?? osHomedir();
}
...
function createPromptApiState(
  ...
  return {
    ...
    loginJobs: new Map(),
    settings: { rotationEnabled: false, retryEnabled: false, retryCount: 1, timeoutMs: 0 },
    rotationIndex: 0,
  };
}
...
function getPromptApiModelOption(modelId: string): PromptApiModelOption | undefined {
...
async function getPromptApiCredentialsPayload(state: PromptApiState) {
  const currentCredentialId = await state.credentialStore.getCurrentCredentialId();
...
function getPromptQuotaSummary(buckets: BucketInfo[] | undefined) {
  const normalizedBuckets = (buckets ?? []).map((bucket) => {
    ...
    return {
      ...
      remaining: remaining !== undefined && !Number.isNaN(remaining)
        ? remaining
        : null,
      limit:
        limit !== undefined && Number.isFinite(limit) && limit > 0 ? limit : null,
  ...
  const numericRemainingBuckets = normalizedBuckets.filter(
    (bucket): bucket is (typeof normalizedBuckets)[number] & { remaining: number } =>
      typeof bucket.remaining === 'number',
  );
  const numericLimitBuckets = normalizedBuckets.filter(
    (bucket): bucket is (typeof normalizedBuckets)[number] & { limit: number } =>
      typeof bucket.limit === 'number',
  );
  const fractionBuckets = normalizedBuckets.filter(
    (
      bucket,
    ): bucket is (typeof normalizedBuckets)[number] & { remainingFraction: number } =>
      typeof bucket.remainingFraction === 'number',
  );
    ...
    new Map(
      normalizedBuckets
        .filter(
          (
            bucket,
          ): bucket is (typeof normalizedBuckets)[number] & { modelId: string } =>
  ...
  const totalRemaining = numericRemainingBuckets.reduce((sum, bucket) => {
    return sum + bucket.remaining;
  }, 0);
  const totalLimit = numericLimitBuckets.reduce((sum, bucket) => {
...
async function getPromptApiCredentialQuotaPayload(
  ...
  try {
    ...
    return {
      ...
      quota: quota as RetrieveUserQuotaResponse,
      quotaSummary,
      sessionPolicy: 'per-request',
    };
  } catch (error) {
...
function normalizeRequestedModel(
  ...
  if (typeof model !== 'string' || model.trim().length === 0) {
    throw new BadRequestError('"model" must be a non-empty string when provided.');
  }
...
async function getEffectiveSourceGeminiCliHome(
  ...
  if (state.settings.rotationEnabled) {
    ...
    if (credentials.length > 0) {
      ...
      logger.info(`[Prompt API] Rotation: using credential "${credential.label}" (${credential.id})`);
  ...
  const credential = await state.credentialStore.getCredential(currentCredentialId);
  if (!credential) {
    return deps.sourceGeminiCliHome;
  }
...
function normalizeCredentialLoginBody(
  body: unknown,
  ...
  if (
    ...
    throw new BadRequestError('"credentialId" must be a non-empty string when provided.');
  }
  ...
  if (
    ...
    throw new BadRequestError('"label" must be a non-empty string when provided.');
  }
...
async function startPromptInvocation(
  ...
  const sourceGeminiCliHome = await getEffectiveSourceGeminiCliHome(deps, state);
...
async function completePromptApiCredentialLogin(
  ...
  if (errorCode) {
    const failedJob = {
      ...
      error: parsedCallbackUrl.searchParams.get('error_description') ?? errorCode,
    };
...
async function runSingleJsonInvocation(
  ...
  deps: Required<PromptApiDependencies>,
  state: PromptApiState,
): Promise<{ assistantText: string; exitCode: number | null; didTimeout: boolean }> {
...
async function handleAdaptedJsonRequest(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    if (attempt > 0) {
      logger.info(`[Prompt API] Retry attempt ${attempt}/${state.settings.retryCount}`);
    }
    ...
    if (result.exitCode === 0) {
      return res
        .status(200)
        .json(adapter.buildJsonResponse(result.assistantText, model, requestId));
    }
...
async function handleAdaptedStreamingRequest(
  ...
  const stdoutDone = consumeOutputLines(child.stdout, (line) => {
    ...
    if (
      ...
      res.write(adapter.formatStreamChunk(event.content, model, requestId, isFirst));
      isFirst = false;
    }
  });
...
export function createPromptApiRouter(
  ...
  const cliEntryPath = getCliEntryPath(workspaceRoot, dependencies.cliEntryPath);
  ...
  router.get('/v1/settings', (_req, res) => {
    res.status(200).json({ settings: state.settings, defaultTimeoutMs: deps.timeoutMs });
  });
    ...
    try {
      ...
      if (b['timeoutMs'] !== undefined) {
        ...
        if (!Number.isFinite(n) || n < 0) {
          throw new BadRequestError('"timeoutMs" must be a non-negative number.');
        }
        state.settings.timeoutMs = Math.floor(n);
      }
      return res.status(200).json({ settings: state.settings, defaultTimeoutMs: deps.timeoutMs });
    } catch (error) {
  ...
  router.get(PROMPT_API_CREDENTIALS_ROUTE, async (_req, res) => {
    try {
      ...
      return res.status(500).json({
        error: error instanceof Error ? error.message : 'Unknown prompt API error',
      });
    }
  });
  ...
  router.put(PROMPT_API_CURRENT_CREDENTIAL_ROUTE, async (req, res) => {
    try {
      ...
      return res.status(200).json({
        currentCredential: getPromptApiCredentialPayload(credential, credential.id),
        sessionPolicy: 'per-request',
      });
    } catch (error) {
      if (error instanceof BadRequestError) {
  ...
  router.get(PROMPT_API_QUOTA_ROUTE, async (req, res) => {
    try {
      return res.status(200).json(
        await getPromptApiCredentialQuotaPayload(state, req.params.credentialId),
      );
    } catch (error) {
      if (error instanceof BadRequestError) {
  ...
  router.put(PROMPT_API_CURRENT_MODEL_ROUTE, (req, res) => {
    try {
      ...
      if (!Object.hasOwn(req.body, 'model')) {
        throw new BadRequestError('A non-empty string "model" field is required.');
      }
  ...
  router.post(PROMPT_API_GEMINI_GENERATE_ROUTE, async (req, res) => {
    try {
      return await handleAdaptedJsonRequest(req, res, geminiAdapter, deps, state);
    } catch (error) {
      if (error instanceof BadRequestError) {
        return res.status(400).json(
          geminiAdapter.buildJsonError(error.message, 400, state.currentModel, ''),
        );
      }
      logPromptApiError(error);
      return res.status(500).json(
  ...
  router.post(PROMPT_API_GEMINI_STREAM_ROUTE, async (req, res) => {
    try {
      return await handleAdaptedStreamingRequest(req, res, geminiAdapter, deps, state);
    } catch (error) {
      if (error instanceof BadRequestError) {
        res.setHeader('Content-Type', geminiAdapter.streamContentType);
        return res.status(400).end(
          geminiAdapter.formatStreamError(error.message, state.currentModel, ''),
        );
      }
      logPromptApiError(error);
      res.setHeader('Content-Type', geminiAdapter.streamContentType);
  ...
  router.post(PROMPT_API_OPENAI_COMPLETIONS_ROUTE, async (req, res) => {
    try {
      if (openaiAdapter.wantsStream(req.body)) {
        return await handleAdaptedStreamingRequest(req, res, openaiAdapter, deps, state);
      }
      return await handleAdaptedJsonRequest(req, res, openaiAdapter, deps, state);
    } catch (error) {
      if (error instanceof BadRequestError) {
        return res.status(400).json(
          openaiAdapter.buildJsonError(error.message, 400, state.currentModel, ''),
        );
      }
      logPromptApiError(error);
      return res.status(500).json(
  ...
  router.put('/v1/auth/token', (req, res) => {
    try {
      ...
      if (typeof newToken !== 'string' || newToken.trim().length === 0) {
        throw new BadRequestError('A non-empty string "token" field is required.');
      }
